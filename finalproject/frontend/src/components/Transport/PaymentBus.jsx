...
const PaymentBus = () => {
  ...
  const { bus, passengers, selectedSeats, passengerDetails } =
    location.state || {};
  const totalAmount = (bus.price * passengers * 1.12).toFixed(2); // 12% tax assumed
  ...
  const handlePayment = async () => {
    ...
    const options = {
      ...
      description: "Booking bus ticket through Razorpay",
      handler: function (response) {
        navigate("/paymentsuccessbuspage", {
          state: {
            bus,
            passengers,
            selectedSeats,
