...
const PaymentTrain = () => {
  ...
  const { train, passengers, passengerDetails } = location.state || {};
  const totalAmount = (train.price * passengers * 1.12).toFixed(2);
  ...
  const handlePayment = async () => {
    ...
    const options = {
      ...
      description: "Booking train ticket through Razorpay",
      handler: function (response) {
        navigate("/paymentsuccesstrainpage", {
          state: {
            train,
            passengers,
            passengerDetails,
