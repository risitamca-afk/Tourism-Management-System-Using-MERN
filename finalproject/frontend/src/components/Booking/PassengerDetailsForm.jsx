...
const PassengerDetailsForm = () => {
  ...
  const { formData, flight } = location.state || {}; // `flight` contains the selected flight details, including price
  ...
  const validateField = (index, field, value) => {
    ...
    if (field === "name") {
      ...
      else if (isNaN(age) || age < 1 || age > 120) error = "Age must be between 1 and 120.";
  ...
  const handlePayment = () => {
    ...
    const options = {
      ...
      amount: 23960 * 100, // Dynamic price in paise (multiply by 100 for rupees)
      currency: "INR",
      name: "BookYourSpot",
      description: `Booking for ${formData.travelers} passengers`,
      image: navlogo, // Replace with your company logo URL
